    ...
    private void setInputData() {
        btnCheckout.setOnClickListener(v -> {
            if (totalItems == 0 || totalPrice == 0) {
                Toast.makeText(OrderActivity.this, "Choose the menu first!",
                        Toast.LENGTH_SHORT).show();
            } else if (totalItems < 2) {
                Toast.makeText(OrderActivity.this, "Minimum 2 orders!",
                        Toast.LENGTH_SHORT).show();
            } else {
                orderViewModel.addDataOrder(strTitle, totalItems, totalPrice);
                Toast.makeText(OrderActivity.this,
                        "Your order is being processed, check the history menu!",
                        Toast.LENGTH_SHORT).show();
                finish();
            }
        });
